...
import { fetchCategories, fetchCategory, saveCategory, saveCategoryVisibility, type CategoryOption } from '@/services/adminApi'
...
export function useToggleCategoryVisibility() {
  ...
  >({
    mutationFn: saveCategoryVisibility,
    onSuccess: (category, variables) => {
      queryClient.setQueryData<{ docs: CategoryOption[] } | undefined>(CATEGORIES_QUERY_KEY, (current) => {
        if (!current) {
          return current
        ...
        return {
          docs: current.docs.map((item) => (item.id === category.id ? category : item)),
        }
      })
